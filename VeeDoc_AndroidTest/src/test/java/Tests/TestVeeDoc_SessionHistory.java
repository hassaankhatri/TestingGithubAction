...
import Pages.PageVeeDoc_SessionHistory;
import Pages.Utility;
import Utilities.Xls_Reader;

public class TestVeeDoc_SessionHistory extends BaseClass{
	...
	@Test(priority=2)
	public void Veedoc_SessionHistory() throws Exception {
		 ...
		 morePage.sessionHistory();
		 test.log(Status.INFO, "Navigate to Search Appointment Page");
		 
		 PageVeeDoc_SessionHistory sh = new PageVeeDoc_SessionHistory(driver);
		 sh.searchInSessionHistory(epname);
		 softAssert.assertEquals(sh.getEndPointName(), epname);
		 test.log(Status.INFO, "Verify Endpoint name is same as Endpoint we connected");

		 System.out.println(sh.getSessionDateTime() + " Session time");
		 System.out.println(epsession.currentDate + " Current");
		 System.out.println(epsession.currentDatePlusOne + " Plus 1");
		 if (sh.getSessionDateTime() == epsession.currentDate) {
			 softAssert.assertEquals(sh.getSessionDateTime(), epsession.currentDate);
			 test.log(Status.INFO, "Verify Endpoint time is same on which we have connected" + epsession.currentDate);
		 }
		 else if (sh.getSessionDateTime() == epsession.currentDatePlusOne){
			 softAssert.assertEquals(sh.getSessionDateTime(), epsession.currentDatePlusOne);
			 test.log(Status.INFO, "Verify Endpoint time is same on which we have connected" + epsession.currentDatePlusOne);
		 }
		 
		 sh.backToDashboard();
		 test.log(Status.INFO, "Search Completed and verified, Back to Dashboard page");
		 
		//functions for Signout
		 dashboard.signOut();
		 test.log(Status.INFO, "Clicked on SignOut Button");
			
		 test.log(Status.INFO, "Veedoc Session History Test Completed");
		 softAssert.assertAll();
	}
}
